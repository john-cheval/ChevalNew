...
const MobileServicesNew = ({serviceList}) => {
  ...
  const initialActiveService = serviceList?.find((servic)=>servic.name.toLowerCase() === 'craft')
       ...
       services ?.map(({ id, ref, data, banner }) => {
        return(
           <div key={id} id={id} ref={ref}>
            ...
            <div>
              {Object.values(data)?.length> 0 && Object.values(data)?.map((item, index) => (
                <Link
                  href={item?.post_name}
                  key={index}
                ...
                >
                  ...
                  <div className="space-y-2 xl:space-y-3">
                    ...
                    {Object.values(item?.service_list_pages  || {})?.length > 0 ?(
                      <div className="text-sm sm:text-base font-satoshi font-normal inline-block">
                      {Object?.values(item?.service_list_pages)?.map((servic, idx) => (
                        <span
                          key={idx}
                          className="text-[#101763] leading-[182%]"
                        >
                          <Link
                           href={`/service/${item?.post_name}/${servic?.post_name}`}
                            > 
                                        {servic?.post_title}
                                         </Link>
                          {idx !== Object?.values(item?.service_list_pages) - 1 && (
                    ...
                    ): <div className="text-sm sm:text-base font-satoshi font-normal inline-block">
                       {
                                  ...
                                  item?.list_page_services?.map((servic, idy) => (
                                      ...
                                      >
                                        <Link
                                        href={`/service/${item?.post_name}`}
                                      > 
                                        {servic}
                                      
                                         </Link>
                                        {idy !==
                                             item?.list_page_services?.length -
                                          1  && (
