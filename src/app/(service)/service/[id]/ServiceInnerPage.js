...
function ServicesInnerPage({ data, faq_title, faq_list, services }) {
  return (
    <>
      ...
      <div className={`grid ${data?.form_area_html ? 'grid-cols-12':'grid-cols-1'}  gap-x-5 bg-white`}>
      ...
      />
        ...
        {data?.form_area_html && (
  <div className={`col-span-12 lg:col-span-6 px-5 sm:px-10 md:px-12 ${data?.form_area_html ? ' lg:pt-24 pb-12':'pt-0'}`}>
<div className="locationListService" dangerouslySetInnerHTML={{__html:data?.form_area_html}}/>
        </div>
        )}
      </div>
